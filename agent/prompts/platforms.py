...
AUTHOR_CONTEXT is already injected via the base system prompt (AGENT_SYSTEM_PROMPT),
so platform prompts focus only on platform-specific guidelines.
"""


BLOG_PROMPT = """블로그(네이버)는 호두님의 '개인 욕망을 펼치는 공간'이다.
...
BRUNCH_PROMPT = """브런치는 커리어·성장을 다루는 전문가 채널이다. 진지하되 친근하고, 전문적이되 솔직한 톤.
...
INSTAGRAM_PROMPT = """인스타그램(@hodu_thecreator)은 개인 욕망 채널. 바이오: "가정부 및 집사 / 호주머니의 사생활 👀"
...
LINKEDIN_PROMPT = """링크드인은 커리어 전문가 채널. 주 독자: 디자인 업계 종사자, 시니어 디자이너, 리더십 고민하는 직장인.
